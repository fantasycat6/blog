...
![image.png](https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/2446/1653958466007/575093883cd2423ab81eb7a2d89571a4.png)

1. 到这里我们就已经避免了iat表中出现敏感函数，但是如果再极端一点，杀毒软件做了非常严格的限制，以至于LoadLibraryW函数也被监视并限制，在前面我们看到了，哪怕不使用getprocaddress函数，它也会出现在iat表中，我们知道了getprocaddress是存放在kernel32.dll中的，那就存在一个问题，我们如何在不实用LoadLibraryW函数的情况下获取Kernel32的地址。 到这里我们的大致思路已经清晰了。
2. 获取kernel32.dll 基地址；
3. 定位 GetProcAddress函数的地址；
4. 使用GetProcAddress确定 LoadLibrary函数的地址；
5. 然后使用 LoadLibrary加载DLL文件（例如user32.dll）；
6. 使用 GetProcAddress查找某个函数的地址（例如MessageBox）；
7. 指定函数参数；
8. 调用函数。
...
![image.png](https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/2446/1653958466007/d2c4ee0ba315432ebaf8c166aed62259.png)
